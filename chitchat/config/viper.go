...
func init() {
	...
	// 监听配置文件变更
	/*
		通过 runtimeViper.WatchConfig() 方法监听配置文件变更（该监听会开启新的协程执行，不影响和阻塞当前协程），一旦配置文件有变更，
		即可通过定义在 runtimeViper.OnConfigChange 中的匿名回调函数重新加载配置文件并将配置值映射到 ViperConfig 指针，同时再次加载新的语言文件。
	*/
